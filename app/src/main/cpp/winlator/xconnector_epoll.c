...
Java_com_steamdeck_mobile_core_xconnector_XConnectorEpoll_createAFUnixSocket(JNIEnv *env, jobject obj,
                                                                jstring path) {
    ...
    const char *pathPtr = (*env)->GetStringUTFChars(env, path, NULL);
    ...
    if (bind(fd, (struct sockaddr*) &serverAddr, addrLength) < 0) {
        __android_log_print(ANDROID_LOG_ERROR, "XConnectorEpoll", "bind() failed: errno=%d, path=%s", errno, serverAddr.sun_path);
        goto error;
    }
    if (listen(fd, MAX_EVENTS) < 0) {
        __android_log_print(ANDROID_LOG_ERROR, "XConnectorEpoll", "listen() failed: errno=%d", errno);
        goto error;
    }

    __android_log_print(ANDROID_LOG_INFO, "XConnectorEpoll", "Socket created successfully: %s", serverAddr.sun_path);
    return fd;
    error:
    close(fd);
    return -1;
...
JNIEXPORT jint JNICALL
Java_com_steamdeck_mobile_core_xconnector_XConnectorEpoll_createEpollFd(JNIEnv *env, jobject obj) {
    return epoll_create(MAX_EVENTS);
}

JNIEXPORT void JNICALL
Java_com_steamdeck_mobile_core_xconnector_XConnectorEpoll_closeFd(JNIEnv *env, jobject obj, jint fd) {
    close(fd);
}

JNIEXPORT jboolean JNICALL
Java_com_steamdeck_mobile_core_xconnector_XConnectorEpoll_doEpollIndefinitely(JNIEnv *env, jobject obj,
    ...
    for (int i = 0; i < numFds; i++) {
        if (events[i].data.fd == serverFd) {
            ...
            if (clientFd >= 0) {
                __android_log_print(ANDROID_LOG_INFO, "XConnectorEpoll", "X11 client connected: fd=%d", clientFd);
                if (addClientToEpoll) {
...
JNIEXPORT jboolean JNICALL
Java_com_steamdeck_mobile_core_xconnector_XConnectorEpoll_addFdToEpoll(JNIEnv *env, jobject obj,
                                                          jint epollFd,
                                                          jint fd) {
    struct epoll_event event;
    event.data.fd = fd;
...
JNIEXPORT void JNICALL
Java_com_steamdeck_mobile_core_xconnector_XConnectorEpoll_removeFdFromEpoll(JNIEnv *env, jobject obj,
                                                               jint epollFd, jint fd) {
    epoll_ctl(epollFd, EPOLL_CTL_DEL, fd, NULL);
}

JNIEXPORT jint JNICALL
Java_com_steamdeck_mobile_core_xconnector_ClientSocket_read(JNIEnv *env, jobject obj, jint fd, jobject data,
...
JNIEXPORT jint JNICALL
Java_com_steamdeck_mobile_core_xconnector_ClientSocket_write(JNIEnv *env, jobject obj, jint fd, jobject data,
                                                jint length) {
    char *dataAddr = (*env)->GetDirectBufferAddress(env, data);
...
JNIEXPORT jint JNICALL
Java_com_steamdeck_mobile_core_xconnector_XConnectorEpoll_createEventFd(JNIEnv *env, jobject obj) {
    return eventfd(0, EFD_NONBLOCK);
}

JNIEXPORT jint JNICALL
Java_com_steamdeck_mobile_core_xconnector_ClientSocket_recvAncillaryMsg(JNIEnv *env, jobject obj, jint clientFd, jobject data,
                                                           jint offset, jint length) {
    char *dataAddr = (*env)->GetDirectBufferAddress(env, data);
...
JNIEXPORT jint JNICALL
Java_com_steamdeck_mobile_core_xconnector_ClientSocket_sendAncillaryMsg(JNIEnv *env, jobject obj, jint clientFd,
...
JNIEXPORT jboolean JNICALL
Java_com_steamdeck_mobile_core_xconnector_XConnectorEpoll_waitForSocketRead(JNIEnv *env, jobject obj, jint clientFd, jint shutdownFd) {
