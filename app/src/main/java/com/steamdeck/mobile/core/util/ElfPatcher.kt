 ...
 *
 * Background:
 * Android 5.0+ (Lollipop and later) only supports Position Independent Executable (PIE).
 * Binaries with e_type ET_EXEC (2) in ELF header cannot execute due to "unexpected e_type" error.
 * By changing e_type to ET_DYN (3), Android linker will accept it.
 *
 * ELF Header Structure (64-bit):
 * - Offset 0x00: e_ident[16] (Magic number: 0x7F 'E' 'L' 'F')
 * - Offset 0x10: e_type (2 bytes) - Executable file type
 * - ET_EXEC = 2 (Non-PIE executable) - Not supported on Android 5.0+
 * - ET_DYN = 3 (Shared library/PIE executable) - Required for Android
 *
 * References:
...
object ElfPatcher {
 ...
 /**
  * Little-endian read helper functions
  */
 ...
 /**
  * Convert ELF binary to Android-compatible (PIE)
  *
  * Patch to ET_DYN format if binary is in ET_EXEC format.
  * This satisfies Android 5.0+ PIE requirements.
  *
 ...
 /**
  * Check if binary is in ET_EXEC format
  *
  * @param binaryFile ELF binary to check
  * @return true: ET_EXEC (patch required), false: ET_DYN or other
  */
  ...
  * to path corresponding to app package directory.
  *
  * @param binaryFile ELF binary file to patch
 ...
 /**
  * Adapt ELF binary TLS segment alignment for Android 12+
  *
  * Android 12 (API 31+) Bionic linker requires TLS segment alignment
  * of minimum 64 bytes. Many binaries use 16 bytes and crash at runtime.
  *
  * @param binaryFile ELF binary file to patch
  * @param requiredAlignment Required alignment (default: 64)
  * @return Result<Unit> Success or failure
  */
